...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const mintper_input = ip_parsed.limit;
    ...
    const mintper_value = Number.parseInt(Number(mintper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (mint per interval):{' '}
                  {mojo_to_mint_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = mint_to_mojo(fee_input.value);
